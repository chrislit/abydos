...
class Arithmetic(object):
    ...
    def train(self, text):
        ...
        for char, count in sorted(
            ...
            self.probs[char] = (prev, follow)
            prev = follow
            tot = tot + count

    def encode(self, text, probs=None):
    ...
    def decode(self, longval, nbits, probs=None):
        ...
        if probs is not None:
            self.probs = probs
        probs_items = [
            (char, minval, maxval) for (char, (minval, maxval)) in self.probs.items()
        ]
...
def ac_encode(text, probs):
    ...
    """
    coder = Arithmetic()
    return coder.encode(text, probs)
...
def ac_decode(longval, nbits, probs):
    ...
    """
    coder = Arithmetic()
    return coder.decode(longval, nbits, probs)
